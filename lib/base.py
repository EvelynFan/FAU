...
from functions import generate_batch,denormalize_input,get_lr
...
class Base(object):
    ...
    def load_weights(self, model=None):
        if model == 'last_epoch':
            sfiles = os.path.join(self.cfg.model_dir, 'snapshot_*.ckpt.meta')
        ...
        if isinstance(model, int):
            model = os.path.join(self.cfg.model_dir, 'snapshot_%d.ckpt' % model)

        if isinstance(model, str) and (os.path.exists(model + '.meta') or os.path.exists(model)):
...
class Trainer(Base):
    ...
    def _make_data(self):
        ...
        return dataiter, math.ceil(len(train_data)/self.cfg.batch_size) 
    ...
    def train(self):
        ...
        train_saver = Saver(self.sess, tf.global_variables(), self.cfg.model_dir)
...
class Tester(Base):
    ...
    def next_feed(self, batch_data=None):
        ...
        if batch_data is None:
            ...
            if self._input_list[0][0].get_shape().as_list()[0] is None:
                ...
                assert self.cfg.test_batch_size == len(batch_data[0]), \
    ...
    def predict_one(self, data=None,itr=0):
        ...
        if self.cfg.vis and self.cfg.test_batch_size == 1:
            tmpimg = np.squeeze(data[0])
            ...
            for i in range(self.cfg.num_AU_points): #10,24
                ...
                if i%2 == 0:
                    heatmap_ = np.uint8(heatmap)
                    final_img = cv2.applyColorMap(cv2.resize(heatmap_,(256,256)), cv2.COLORMAP_JET)
                else:
                    heatmap_ = np.uint8(heatmap)
        ...
        if data is not None and len(data[0]) <  batch_size:
